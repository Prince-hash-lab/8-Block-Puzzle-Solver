...
{  cout<<" in closed >>>><<<<<<<<<<<>>>>>>>>><<<<<<"<<endl;
   ...
   for (ptr = tclosed.begin(); ptr != tclosed.end(); ptr++)
       { printMatrix((*ptr)->mat);
           if(compmat((*ptr)->mat,ngbr->mat)) {cout<<"yes true #$#*$#(*$#$(#*$#(*(*$#("<<endl;return true;}         
       }
     cout<<" false hai "<<" )(@(#@)#(@)#(@)#@@!)!))#))#$)$#%)%)%)%)%)%)($($(((%*%&$&%*$&*$&*%*%"<<endl;
    return false;
}

void bfs(int mat1[n][n],int k , int l, int mat2[n][n],node* parent)
{   vector<node*> que;
    vector<node*> closed;
    size=0;
    ...
    {   sort(que.begin(),que.end(),mycmp);
        ...
        int hcurr=curr->hval;
            ...
            {  cout<<" in 1st if "<<endl;
                ...
                if(!inclosed(closed,arr[size])) //to check if it stucked in loop or not// if stuck in loop then dont add in queue
                         {  que.push_back(arr[size]);
                         cout<<" in if _________))_)_)_)______ closed++++++"<<endl;
...
int main() {
   ...
   //final state
    ...
    node * parent =NULL;
    bfs(initial, x, y, final ,parent);  // calling dfs 
