...
{   vector<node*> que;        // taken as priority queue  प्राथमिकता कतार के रूप में लिया गया
        // this is vector of closed states यह बंद स्थिति का वेक्टर है
    size=0;
        ...
        int hcurr=curr->hval;
         if(issafe(val1,val2-1) &&   val4!=(val2-1) )
            ...
            {  
                ...
                arr[size]->hval=calheruistic(arr[size],mat2);
                if(arr[size]->hval<hcurr)
                         // check if it is inclosed or not 
                         {  que.push_back(arr[size]);
          ...
          if(issafe(val1,val2+1) &&   val4!=(val2+1) )
            ...
            cout<<" no solution using Hill climbing "<<endl;
